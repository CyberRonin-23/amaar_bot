...
def send_info_to_group(message:Message):
    ...
    btn  = InlineKeyboardButton("Mijoz bilan bog'landim✅",callback_data=f"recontacted:|{user_id}|{message.contact.phone_number}")
    ...
    bot.send_message(GROUP_ID,f"""Mijoz TelegramID: {user_id}
Mijozning Username : @{message.from_user.username if message.from_user.username else "No username"}
Mijozning kontakti: +{message.contact.phone_number}
    
    
Status: Bog'lanilmagan❌""",reply_markup=markup)
